...
int ifping;
float fping, pingval;
// Preprocessor directives #define NAME CONSTANT
...
/////////////////////////////////// Clue Important  ////////////////////////////////
//float gtrim = 6.1; // T-BotW
float gtrim = 1.70; // T-Bot0
//float gtrim = 0.85; // T-Bot
//float gtrim = 1.0; // T-Bot-LC
float rtrim = 0;
...
double speedKp=0.09, speedKi=0, speedKd=0.0, KPS = 0.09, KP = 4, KI = 85, KPS_last, KP_last, KI_last; // do not delete
double gyroKp=4.0, gyroKi=85, gyroKd=0.0; // do not delete
...
Task printData(100, TASK_FOREVER, &printDataCallBack);
Scheduler runner;
...
void uSoundCallBack(){    
  ...
  if (pingval > 0.1 && pingval < MAX_DISTANCE+1){
   backoff = -50;
    ...
    //NewTone(buzzer, int(abs(kalAngleY*10)));
}
