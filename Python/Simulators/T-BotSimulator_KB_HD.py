...
stick_man = tuple(map(tuple, tuple((stick_man+[1350-stick_man_h_centre,origin[1]+wheel_radius-stick_man[:,1].max()]).astype(int))))
speedfactor = 0.6
speedlimit = 65
turnspeedlimit = 70
oldvals = [0,0,0,0]
...
while not done:
    ...
    if theta >= -np.pi/2.2 and theta <= np.pi/2.2:
        ...
        origin[0] = 1000+int(distance*mm2px)+int(((theta)*np.pi)*wheel_radius/4)       
    ...
    pygame.gfxdraw.filled_polygon(screen, (track_marks_tup), (0, 249, 249, 100))
    ...
    textPrint.abspos(screen, "Tuning Parameters",(10,740))
    textPrint.tprint(screen, " ")
    ...
    textPrint.abspos(screen, "g: {:.2f}".format((g)),(1800,10))
